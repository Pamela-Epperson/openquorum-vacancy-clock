// South Carolina — SOS "List of Vacancies on Statewide and Multi-County
// Boards" PDF (search.scsos.com). Conservative line parser (WA/AZ doctrine).
// VERIFY OUTPUT ON FIRST ACTIONS RUN.
import { classifyDomain } from "../lib/domains.mjs";
import { browserFetch } from "../lib/http.mjs";

const NOISE_START = /^(The|A|An|This|It|In|As|Page|Report|Updated|List|Position|Vacanc|Seat|Member|Members|Term|Expir|Total|South Carolina Secretary|Secretary of State)\b/i;

export async function scrape({ endpoint, applyUrl, authority }) {
  const { default: pdfParse } = await import("pdf-parse/lib/pdf-parse.js");
  const res = await browserFetch(endpoint);
  if (!res.ok) throw new Error(`SC PDF ${res.status}`);
  ...
  for (const raw of text.split("\n")) {
    ...
    const row = { id: id++, name, domain: classifyDomain(name),
      totalSeats: null, vacantSeats: 1, vacantSince: null,
      authority, constituent: null, applyUrl,
      sourceUrl: endpoint, lastVerified: today,
      criticalNote: "Listed in SC Secretary of State vacancy list" };
    seen.set(name, row); rows.push(row);
  }
  if (rows.length === 0) throw new Error("SC parser found no rows — PDF layout may have changed");
  return rows;
}
